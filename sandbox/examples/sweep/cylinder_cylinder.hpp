...
class CylinderCylinderSweepExample : public IExample {
public:
...
inline void CylinderCylinderSweepExample::OnUpdate(float delta_time) {}

inline void CylinderCylinderSweepExample::OnRender3D(float delta_time) {
    ...
    Matrix rot2 = MatrixRotateXYZ({m_r2.x * DEG2RAD, m_r2.y * DEG2RAD,
                                   m_r2.z * DEG2RAD});
    ::Vector3 axis2 = Vector3Normalize(Vector3Transform({0, 1, 0}, rot2));

    toy_physics::CylinderSupportFunction s1(ToVec3(m_c1), ToVec3(axis1), m_hh1, m_rv1);
    toy_physics::CylinderSupportFunction s2(ToVec3(m_c2), ToVec3(axis2), m_hh2, m_rv2);
    m_status = toy_physics::Sweep(s1, s2, ToVec3(dir), m_sweep.m_len, true, &m_t,
                                  &m_pos, &m_nrm, &m_w1, &m_w2);

    m_ctx.DrawCylinder(m_c2, m_r2, m_hh2 * 2.f, m_rv2, WHITE);
    m_ctx.DrawCylinder(m_c1, m_r1, m_hh1 * 2.f, m_rv1, BLUE);
    if (m_status == 1) {
        m_ctx.DrawCylinder(
...
inline void CylinderCylinderSweepExample::OnRender2D(float delta_time) {
