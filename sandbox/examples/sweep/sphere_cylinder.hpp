...
class SphereCylinderSweepExample : public IExample {
public:
...
inline void SphereCylinderSweepExample::OnUpdate(float delta_time) {}

inline void SphereCylinderSweepExample::OnRender3D(float delta_time) {
    ...
    toy_physics::CylinderSupportFunction s2(ToVec3(m_c2), ToVec3(axis2), m_hh2, m_r2v);
    m_status = toy_physics::Sweep(s1, s2, ToVec3(dir), m_sweep.m_len, true, &m_t,
                                  &m_pos, &m_nrm, &m_w1, &m_w2);

    m_ctx.DrawCylinder(m_c2, m_r2, m_hh2 * 2.f, m_r2v, WHITE);
...
inline void SphereCylinderSweepExample::OnRender2D(float delta_time) {
