...
class CylinderBVExample : public IExample {
public:
...
inline void CylinderBVExample::OnUpdate(float delta_time) {}

inline void CylinderBVExample::OnRender3D(float delta_time) {
    m_ctx.DrawCylinder(m_center, m_rotation, m_half_height * 2.f, m_radius);

    toy_physics::CylinderGeometry cylinder{m_radius, m_half_height};
    auto bv = toy_physics::BuildBV(cylinder, EulerToQuat(m_rotation));
    bv.m_min += ToVec3(m_center);
    bv.m_max += ToVec3(m_center);
    DrawAABBWire(bv);
}

inline void CylinderBVExample::OnRender2D(float delta_time) {
    ...
    GuiGroupBox({x, y, kPanelW, kRowH * 6 + 30}, "Cylinder");
    y += kRowH;
