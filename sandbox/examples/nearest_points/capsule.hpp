...
class CapsuleNearestPointExample : public IExample {
public:
...
inline void CapsuleNearestPointExample::OnUpdate(float delta_time) {}

inline void CapsuleNearestPointExample::OnRender3D(float delta_time) {
    ::Vector3 p = m_point.ToCartesian(m_center);

    float height = m_half_height * 2.f;
    m_ctx.DrawCapsule(m_center, m_rotation, height, m_radius, BLUE);
    ...
    auto np = toy_physics::GetCapsuleNearestPoint(
        ToVec3(p), ToVec3(m_center), ToVec3(dir), m_half_height, m_radius);
    ::Vector3 nearest = FromVec3(np);
    m_ctx.DrawSphere(nearest, {0, 0, 0}, 0.3f, PURPLE);
    DrawLine3D(p, nearest, GREEN);
}

inline void CapsuleNearestPointExample::OnRender2D(float delta_time) {
    ...
    // --- Capsule ---
    GuiGroupBox({x, y, kPanelW, kRowH * 9 + kSecPad + kPad}, "Capsule");
    y += kRowH;
    ...
    m_point.RenderGUI(x, y, kPanelW, kLabelW, kCtrlH, "Point P (rel. Capsule)");
