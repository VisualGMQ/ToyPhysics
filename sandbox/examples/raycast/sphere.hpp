...
class SphereRaycastExample : public IExample {
public:
...
inline void SphereRaycastExample::OnUpdate(float delta_time) {}

inline void SphereRaycastExample::OnRender3D(float delta_time) {
    ...
    m_hit_count = toy_physics::RaycastSphere(
        ToVec3(origin), ToVec3(dir), m_ray.m_len, ToVec3(m_center), m_radius,
        m_hits, flags);
    DrawRayHits(origin, m_hits, m_hit_count);
}

inline void SphereRaycastExample::OnRender2D(float delta_time) {
