...
class GjkTetrahedronsExample : public IExample {
public:
...
inline void GjkTetrahedronsExample::OnUpdate(float delta_time) {
    if (m_success_timer > 0) m_success_timer -= delta_time;
}

inline ::Vector3 ComputeTetVertex(::Vector3 c, ::Vector3 rot, float size,
...
inline void DrawTetFaces(::Vector3 w[4], Color clr) {
    int faces[4][3] = {
        {1, 2, 3},
        {0, 3, 2},
        {0, 1, 3},
    ...
    for (int i = 0; i < 4; ++i)
        DrawTriangle3D(w[faces[i][0]], w[faces[i][1]], w[faces[i][2]], clr);
}

inline void DrawTetEdges(::Vector3 w[4], Color clr) {
    for (int i = 0; i < 4; ++i)
        for (int j = i + 1; j < 4; ++j) DrawLine3D(w[i], w[j], clr);
}

inline void GjkTetrahedronsExample::OnRender3D(float delta_time) {
    ::Vector3 local[4] = {
        { 1,  1,  1},
        { 1, -1, -1},
        {-1,  1, -1},
    ...
    ::Vector3 w1[4], w2[4];
    for (int i = 0; i < 4; ++i) {
        w1[i] = ComputeTetVertex(m_c1, m_r1, m_size1, m_scale1[i], local[i]);
        w2[i] = ComputeTetVertex(m_c2, m_r2, m_size2, m_scale2[i], local[i]);
    }

    Color c = m_result ? RED : BLUE;
    Color faceColor = m_result ? Color{255, 0, 0, 51} : Color{0, 0, 255, 51};
    DrawTetFaces(w1, faceColor);
    DrawTetFaces(w2, faceColor);
    DrawTetEdges(w1, c);
    DrawTetEdges(w2, c);
    for (int i = 0; i < 4; ++i) {
        ::DrawSphere(w1[i], 0.2f, c);
        ::DrawSphere(w2[i], 0.2f, c);
    }

    if (!m_has_points) return;

    if (m_result) {
        ::DrawSphere(FromVec3(m_p1), 0.3f, MAGENTA);
    } else {
        ::Vector3 p1 = FromVec3(m_p1);
        ::Vector3 p2 = FromVec3(m_p2);
        ::DrawSphere(p1, 0.25f, GREEN);
        ::DrawSphere(p2, 0.25f, DARKGREEN);
        DrawLine3D(p1, p2, GREEN);
    }
}

inline void GjkTetrahedronsExample::OnRender2D(float delta_time) {
    ...
    toy_physics::Vector3 init_dir = ToVec3(m_c2) - ToVec3(m_c1);
    if (init_dir.squaredNorm() <= toy_physics::kREAL_EPSILON) {
        init_dir = toy_physics::Vector3::UnitX();
    }

    m_result = toy_physics::IsIntersect(ps1, ps2, init_dir,
                                        toy_physics::kDefaultRealTolerance);

    m_has_points = false;
    if (m_result) {
        m_has_points = toy_physics::CalcCommonPoint(
            ps1, ps2, init_dir, m_p1, m_p2, toy_physics::kDefaultRealTolerance);
    } else {
        (void)toy_physics::CalcClosestPoints(
            ps1, ps2, m_p1, m_p2, toy_physics::kREAL_MAX,
            toy_physics::kDefaultRealTolerance);
        m_has_points = true;
    }
    ...
    y += 4;
    Color textClr = m_result ? RED : GREEN;
    DrawText(m_result ? "INTERSECT" : "SEPARATED", (int)x + 4, (int)y, 26,
             textClr);
