...
bool IsRayAABBIntersect(const Ray& r, real t0, real t1, Vector3 parameters[2]) {
    real tmin =
        (parameters[r.m_sign[0]].x() - r.m_origin.x()) * r.m_inv_direction.x();
    real tmax = (parameters[1 - r.m_sign[0]].x() - r.m_origin.x()) *
                r.m_inv_direction.x();

    real tymin =
        (parameters[r.m_sign[1]].y() - r.m_origin.y()) * r.m_inv_direction.y();
    real tymax = (parameters[1 - r.m_sign[1]].y() - r.m_origin.y()) *
                 r.m_inv_direction.y();
    ...
    real tzmin =
        (parameters[r.m_sign[2]].z() - r.m_origin.z()) * r.m_inv_direction.z();
    real tzmax = (parameters[1 - r.m_sign[2]].z() - r.m_origin.z()) *
                 r.m_inv_direction.z();
